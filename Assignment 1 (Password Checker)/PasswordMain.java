...
public class PasswordMain extends BorderPane
{
	...
	private Label instruction5Label, instruction6Label, instruction7Label;
	...
	public PasswordMain()
	{
		...
		instruction6Label = new Label("\t5.  Must contain at least one special charcter");
		instruction7Label = new Label("\t6.  May not have more than 2 of the same character in sequence");
		...
		subpanel.getChildren().addAll(instruction1Label, instruction2Label, instruction3Label,
				instruction4Label, instruction5Label, instruction6Label,instruction7Label	);
			...
			if(selectedFile != null)
			   {
				...
				illegalPasswords = PasswordCheckerUtility.getInvalidPasswords(passwords);
				for(String passwordString : illegalPasswords)
					results += passwordString +"\n";
				alert.setContentText("Invalid Passwords\n"+results);
				alert.showAndWait();
